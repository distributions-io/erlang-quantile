...
function quantile( p, options ) {
	...
	if ( isNumber( p ) ) {
		return quantile1( p, opts.k, opts.lambda );
	}
	if ( isMatrixLike( p ) ) {
		if ( opts.copy !== false ) {
		...
		return quantile5( out, p, opts.k, opts.lambda );
	}
	if ( isTypedArrayLike( p ) ) {
		...
		return quantile6( out, p, opts.k, opts.lambda );
	}
	if ( isArrayLike( p ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return quantile4( p, opts.k, opts.lambda, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return quantile3( out, p, opts.k, opts.lambda, opts.accessor );
		}
		return quantile2( out, p, opts.k, opts.lambda );
	}
	return NaN;
