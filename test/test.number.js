...
describe( 'number quantile', function tests() {
	...
	it( 'should evaluate the quantile function of the Erlang distribution', function test() {
		var actual;
		for ( var i = 0; i < data.length; i++ ) {
			actual =  quantile( data[ i ], k, lambda );
	...
	it( 'should return `NaN` if provided `NaN` as input', function test() {
		assert.isTrue( isnan( quantile( NaN, k, lambda ) ) );
	});

	it( 'should return `NaN` if provided a number outside [0,1]', function test() {
		assert.isTrue( isnan( quantile( 1.1, k, lambda ) ) );
		assert.isTrue( isnan( quantile( -0.1, k, lambda ) ) );
	});
